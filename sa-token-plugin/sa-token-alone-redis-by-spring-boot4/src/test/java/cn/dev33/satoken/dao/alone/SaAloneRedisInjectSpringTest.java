...
/**
 * Boot4 自动装配应该用 sa-token.alone-redis 重连 Dao，和业务 Redis 隔开
 *
 * @author click33
 * @since 1.46.0
 */
...
public class SaAloneRedisInjectSpringTest {
	...
	/** 业务 Redis 走 spring.data.redis，Sa-Token 走 sa-token.alone-redis */
	@DynamicPropertySource
	static void redisProps(DynamicPropertyRegistry registry) {
		registry.add("spring.data.redis.host", () -> "127.0.0.1");
		registry.add("spring.data.redis.port", () -> businessRedis.getBindPort());
	...
	/** 自动装配后应该拿到 RedisTemplate Dao，并且连到独立 Redis 端口 */
	@Test
	void autoConfig_shouldReinitRedisTemplateDaoAgainstAlonePort() {
		Assertions.assertTrue(saTokenDao instanceof SaTokenDaoForRedisTemplate);
	...
	@SpringBootApplication(exclude = SaTokenDaoForRedisTemplateUseJdkSerializer.class)
	static class App {

		/** 业务 Redis 工厂：jedis-mock 必须 RESP2 */
		@Bean
